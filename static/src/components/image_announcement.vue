<template>
  <div>
    ...
    <markdown-editor v-model="data.body" />
    <div class="form-group">
      <label
        for="level"
        class="control-label"
...
export default {
  ...
  computed: {
    ...
    puturl () {
      if (this.announcement_id) { return '/api/announcement/' + this.announcement_id; } else return '/api/announcement';
    },
    canSave () {
      return !(
        this.data.title === '' ||
        this.data.body === '' ||
        this.data.info.level === ''
      );
    },
    canPublish () {
      return !(
  ...
  created () {
    ...
    if (url.indexOf('/update') !== -1) {
      ...
      console.log(url);
      update = true;
      document.getElementById('announcementtitle').innerHTML =
        'Update announcement';
    }
    var self = this;
    axios({
