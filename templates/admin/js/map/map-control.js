...
$(document).ready(function() {
    ...
    areaSelect.on("change", function() {
        ...
        var neCoords = this.getBounds().getNorthEast().lat + ", " + this.getBounds().getNorthEast().lng;
        $('.ne-bounds').text(neCoords);

        // South West coordinates
        var swCoords = this.getBounds().getSouthWest().lat + ", " + this.getBounds().getSouthWest().lng;
    ...
    function generateAndDownloadCSV(){
        northEastLat = selectedBounds.getNorthEast().lat;
        northEastLon = selectedBounds.getNorthEast().lng;
        southWestLat = selectedBounds.getSouthWest().lat;
        southWestLon = selectedBounds.getSouthWest().lng;
...
function latLongZoomToPixelCoords(lat, lon){
    ...
    var x = ((lon + 180) / 360) * 256 * Math.pow(2, $('.zoom-select').val());
    var y = (0.5 - Math.log((1 + sinLat) / (1 - sinLat)) / (4 * Math.PI)) * 256 * Math.pow(2, $('.zoom-select').val());
...
var tileCoordsAndZoomToQuadKey = function(x, y){
    var quadKey = '';
    for(var i=$('.zoom-select').val(); i>0; i--){
        var digit = 0;
        var mask = 1 << (i - 1);
        if((x & mask) != 0){
            digit += 1
